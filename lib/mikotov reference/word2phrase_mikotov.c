...
void ReadWord(char *word, FILE *fin, char *eof) {
  ...
  while (1) {
    ...
    // \r continues
    if (ch == 13) continue;
    // this is the condition for word boundary
    if ((ch == ' ') || (ch == '\t') || (ch == '\n')) {
...
// It is very important to understand what the authors
// are consdiering to be a vocabulary.
// `struct vocab_word * vocab` is the array that we care about
// it has been vocab_max_size = 10_000
// vocab_hash --> vocab_hash_size == 500*1E6 word hashes
// it will fill up a vocabulary pointer with words and its counts via
// - unigram {word, count}
// - bigram {word1_word2, count}
...
void LearnVocabFromTrainFile() {
  ...
  while (1) {
    // reads one word, from input file, and also sets eof whenever
    // appropriate. It will write a ["\n", "\0"] word for \n occurrences
    // but when it comes to spaces, it will just continue until it finds another 
    // character, it will read the word, and once it gets to a space, it will
    // return, normal split and trim
    ReadWord(word, fin, &eof);
    if (eof) break;
    // if there was a new line, this start flag will be turned on
    if (word[0] == '\n') {
      start = 1;
      continue;
    } else start = 0;
    // how many words did you read already? train words!
    train_words++;
    ...
    i = SearchVocab(word);
    // did I already have this wrod in my vocabulary?
    if (i == -1) { //no
      // add it to the vocabulary, and initialize it
      a = AddWordToVocab(word);
      vocab[a].cn = 1;
    } else vocab[i].cn++; // yes, increment it
    if (start) continue;
    
    // START JOIN
    // Join two words by an underscore
    a = 0;
    b = 0;
    while (last_word[a]) {
...
void TrainModel() {
  ...
  // bigram_word is basically two words
  char word[MAX_STRING], last_word[MAX_STRING], bigram_word[MAX_STRING * 2 + 1], eof = 0;
  real score;
  unsigned long long next_random = 1;
  FILE *fo, *fin;
